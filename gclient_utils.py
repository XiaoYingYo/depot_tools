...
class GClientChildren(object):
  ...
  def KillAllRemainingChildren():
    ...
    with GCLIENT_CHILDREN_LOCK:
      if GCLIENT_CHILDREN:
        print('无法杀死以下子过程:', file=sys.stderr)
...
def CheckCallAndFilter(args, print_stdout=False, filter_fn=None,
 ...
 接受全部 subprocess2.Popen() 参数加:
    print_stdout: 如果为true,则将命令的stdout转发到stdout.
    filter_fn: 一个函数,每行都称为单字符串参数 
                subocess2的输出. 每行都有尾线 
                字符修剪.
    显示标头:是否要在命令输出之前显示标头. 
     始终显示标题:显示标题,即使命令未产生输出. 
     重试:如果该过程退出非零,请短暂睡觉并尝试 
            同样,要重试的最大时间.
    ...
    try:
      while True:
        ...
        try:
          ...
          # 如果我们一段时间没有打印到终端,但是我们已经收到
          # 从惊人的角度出发,让用户知道我们仍在进步.
          now = datetime.datetime.now()
          if (now - self.last_join > datetime.timedelta(seconds=3) and
